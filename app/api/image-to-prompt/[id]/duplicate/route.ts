...
/**
 * POST: Duplicate an image-to-prompt for another user
...
export async function POST(
  ...
  try {
    ...
    if (!originalPrompt) {
      return NextResponse.json({ error: 'Image-to-prompt not found' }, { status: 404 });
    }
    ...
    const duplicatedPrompts = await Promise.all(
      targetUserIds.map(async (targetUserId: string) => {
        ...
        while (await prisma.imageToPrompt.findFirst({
          where: {
            userId: targetUserId,
            name: finalName,
          },
        ...
        const newPrompt = await prisma.imageToPrompt.create({
          data: {
            userId: targetUserId,
            name: finalName,
            prompt: originalPrompt.prompt,
            llmType: originalPrompt.llmType,
    ...
    await logger.success({
      module: 'PROMPT',
      action: 'duplicate',
      message: `Duplicated image-to-prompt "${originalPrompt.name}" for ${targetUserIds.length} users`,
      resourceId: id,
      output: { targetUserIds, duplicatedIds: duplicatedPrompts.map(p => p.id) },
    });

    return NextResponse.json({
      message: `Image-to-prompt duplicated for ${duplicatedPrompts.length} users`,
      duplicatedPrompts,
    });
  } catch (error) {
    console.error('Error duplicating image-to-prompt:', error);
    return NextResponse.json(
      { error: 'Failed to duplicate image-to-prompt' },
      { status: 500 }
    );
  }
}
