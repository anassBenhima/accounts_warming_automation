    ...
    const prompts = await logger.track(
      {
        module: 'PROMPT',
        action: 'list',
        message: 'Fetching keyword search prompts',
      },
      async () => {
        return await prisma.keywordSearchPrompt.findMany({
    ...
    const newPrompt = await logger.track(
      {
        module: 'PROMPT',
        action: 'create',
        message: 'Creating keyword search prompt',
        input: { name },
      },
      async () => {
        return await prisma.keywordSearchPrompt.create({
          data: {
